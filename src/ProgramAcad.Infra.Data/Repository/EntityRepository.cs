...
namespace ProgramAcad.Infra.Data.Repository
{
    public class EntityRepository<TEntity> : IEntityRepository<TEntity> where TEntity : Entity
    {
        ...
        protected EntityRepository(ProgramAcadContext dbContext)
        {
        ...
        public TEntity GetById(Guid id, bool activeOnly = true)
        {
            return _dbSet.Where(x => activeOnly ? x.Status : true).SingleOrDefault(x => x.Id.Equals(id));
        }

        public async Task<TEntity> GetByIdAsync(Guid id, bool activeOnly = true)
        {
            return await _dbSet.Where(x => activeOnly ? x.Status : true).SingleOrDefaultAsync(x => x.Id.Equals(id));
        }
